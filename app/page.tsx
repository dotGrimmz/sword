...
import type { BibleBookPreview, BibleTranslationSummary } from "@/types/bible";

export default async function Page() {
  const supabase = await createClient();

  const { data: webTranslation, error: translationError } = await supabase
  ...
  if (translationError) {
    return (
      <p>❌ Failed to look up WEB translation: {translationError.message}</p>
    );
  }

  const { data: translations, error: listError } = await supabase
    .from("bible_translations")
  ...
  if (webTranslation) {
    const [
      { count: books, error: countBooksError },
      { count: chunks, error: countChunksError },
    ] = await Promise.all([
      supabase
        .from("bible_books")
    ...
    <div className="mx-auto flex min-h-screen max-w-4xl flex-col gap-6 p-10 font-sans">
      <div>
        ...
        <p className="text-sm text-slate-500">
          Run <code>node scripts/seed_bible_WEB.mjs</code> to ingest the World
          English Bible JSON files into Supabase.
        </p>
      </div>
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <ul className="mt-3 divide-y divide-slate-200">
          {((translations ?? []) as BibleTranslationSummary[]).map(
            (translation) => (
              <li
                key={translation.code}
                className="flex flex-col py-3 sm:flex-row sm:items-center sm:justify-between"
              >
                <div>
